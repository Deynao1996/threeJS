...
let index = Math.floor(Math.random() * colorsPalette.length)
// index = 19
...
const GradientShaderMaterial = shaderMaterial(
  ...
  glsl`
    ...
    void main() {
      ...
      for(int i = 0; i < 4; i++) {
        float noiseFlow = 0.03 * float(i);
        float noiseSpeed = 0.06 * float(i);
      ...
      float noise = snoise(vec3(noiseCoord.x + uTime*0.03, noiseCoord.y, uTime * 0.1));
      noise = max(0., noise);
...
const Gradient = () => {
  ...
  return (
    <>
      <mesh>
        <planeBufferGeometry args={[5, 5, 300, 300]} />
        <gradientShaderMaterial ref={materialRef} side={THREE.DoubleSide} />
      </mesh>
    </>
  )
}

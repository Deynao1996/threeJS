...
import { PixiShaderMaterial } from '../ExtendableShaders/PixiShader'
...
const Pixi = () => {
  ...
  const [image, bg, mask, testImg] = useLoader(THREE.TextureLoader, [
    logo,
    background,
    blob,
    test
  ...
  return (
    <>
      ...
      <group>
        <mesh scale={size} position={[0, 0, 0.001]} onPointerMove={onMouseMove}>
        ...
        <group>{createPortal(blobs, scene)}</group>
      </group>
    </>
  )
}

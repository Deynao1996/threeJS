...
const HeadShaderMaterial = shaderMaterial(
  {
  ...
  glsl`
    ...
    void main() {
      ...
      frensel = frensel*frensel*frensel;

      vec2 newFakeUV = vec2((angle + PI)/(2.*PI), phi/PI);
      vec2 fakeUV = vec2(dot(vec3(1.), vNormal), dot(vec3(-1., 0., 1.), vNormal));
      fakeUV = abs(fakeUV);

      vec4 color = texture2D(uTexture, fakeUV + 0.2*cnoise(vec4(fakeUV*10., uTime, 0.), vec4(10.)));
...
const CustomMesh = (props) => {
  const [ image ] = useLoader(THREE.TextureLoader, [sky])
